...
object BluetoothControllerTEST: BluetoothHidDevice.Callback(), BluetoothProfile.ServiceListener {
    ...
    override fun onAppStatusChanged(pluggedDevice: BluetoothDevice?, registered: Boolean) {
        ...
        if(registered)
        {
            ...
            if(btHid?.getConnectionState(pluggedDevice) == BluetoothProfile.STATE_DISCONNECTED && pluggedDevice != null && autoPairFlag == true)
            {
                btHid?.connect(pluggedDevice)
            }

            else if(btHid?.getConnectionState(pairedDevices?.get(0)) == BluetoothProfile.STATE_DISCONNECTED && autoPairFlag == true)
            {
    ...
    private val sdpRecord by lazy {
        BluetoothHidDeviceAppSdpSettings(
            ...
            JoystickDescriptorsCollction.JOYSTICK_DESCRIPTOR_XBOX360
        )
    }
